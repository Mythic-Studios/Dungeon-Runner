...
public class WeaponForgingScreenHandler  extends ScreenHandler {
    private final Inventory inventory;
    private final PropertyDelegate propertyDelegate;
    public final WeaponForgeBlockEntity blockEntity;

    public WeaponForgingScreenHandler(int syncId, PlayerInventory inventory, BlockPos pos) {
        this(syncId, inventory, inventory.player.getWorld().getBlockEntity(pos), new ArrayPropertyDelegate(2));
    }

    public WeaponForgingScreenHandler(int syncId, PlayerInventory playerInventory,
                                BlockEntity blockEntity, PropertyDelegate arrayPropertyDelegate) {
        super(ModScreenHandlers.WEAPON_FORGING_SCREEN_HANDLER, syncId);
        checkSize((Inventory) blockEntity, 4);
        this.inventory = (Inventory) blockEntity;
        this.propertyDelegate = arrayPropertyDelegate;
        this.blockEntity = ((WeaponForgeBlockEntity) blockEntity);

        this.addSlot(new Slot(inventory, 0, 23, 34));
        this.addSlot(new Slot(inventory, 1, 60, 34));
        this.addSlot(new Slot(inventory, 2, 97, 34));
        this.addSlot(new Slot(inventory, 3, 139, 34));
